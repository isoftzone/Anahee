...
const Sidebar = () => {
    const [currentMenu, setCurrentMenu] = useState(() => localStorage.getItem('currentMenu') || '');
    const [basicSubMenuOpen, setBasicSubMenuOpen] = useState(() => localStorage.getItem('basicSubMenuOpen') === 'true');
    const [bestInSubMenuOpen, setBestInSubMenuOpen] = useState(false);
    const [shouldActivateMenu, setShouldActivateMenu] = useState(true);
    ...
    const toggleMenu = (menu: string) => {
        if (menu === 'basic') {
            ...
            setCurrentMenu(currentMenu === menu ? '' : menu);
        }
    };

    // Load menu states from localStorage on component mount
    useEffect(() => {
        const storedBasicSubMenuState = localStorage.getItem('basicSubMenuOpen');
        const storedBestInSubMenuState = localStorage.getItem('bestInSubMenuState');

        if (storedBasicSubMenuState) {
            setBasicSubMenuOpen(storedBasicSubMenuState === 'true');
        }
        if (storedBestInSubMenuState) {
            setBestInSubMenuOpen(storedBestInSubMenuState === 'true');
        }
    }, []);
    ...
    useEffect(() => {
        localStorage.setItem('basicSubMenuOpen', basicSubMenuOpen.toString());
        localStorage.setItem('bestInSubMenuState', bestInSubMenuOpen.toString());
        localStorage.setItem('currentMenu', currentMenu);
    }, [basicSubMenuOpen, bestInSubMenuOpen, currentMenu]);
    ...
    // Activate current menu based on URL path
    useEffect(() => {
        if (!shouldActivateMenu) return;
        ...
        if (selector) {
            ...
            if (ul) {
                ...
                if (parentMenu) {
                    ...
                    if (navLink) {
                        const onClickAttribute = navLink.getAttribute('onclick');
                        if (onClickAttribute) {
                            const match = onClickAttribute.match(/toggleMenu\(['"]([^'"]+)['"]\)/);
                            if (match && match[1]) {
                                toggleMenu(match[1]);
                            }
                        }
        ...
        setShouldActivateMenu(false);
    }, [shouldActivateMenu, location.pathname]);
                ...
                <div className="bg-white dark:bg-black h-full">
                    ...
                    <PerfectScrollbar className="h-[calc(100vh-80px)] relative">
                        <ul className="relative font-semibold space-y-0.5 p-4 py-0">
                            ...
                            <li className="menu nav-item">
                                <button type="button" className={`${currentMenu === 'datalabels' ? 'active' : ''} nav-link group w-full`} onClick={() => toggleMenu('datalabels')}>
                                    <div className="flex items-center">
                                        <IconMenuDatatables className="group-hover:!text-primary shrink-0" />
                                        <span className="ltr:pl-3 rtl:pr-3 text-black dark:text-[#506690] dark:group-hover:text-white-dark">Reports</span>
                                    </div>
                                    <div className={currentMenu !== 'datalabels' ? 'rtl:rotate-90 -rotate-90' : ''}>
                                        <IconCaretDown />
                                    </div>
                                </button>
